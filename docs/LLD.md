...
## Out of scope for v1
LLM-graded descriptive/essay questions; importing or scraping past papers from any external source (permanently out, not deferred); **bulk import of a teacher's own question bank** (CSV/paste-many — a reasonable v2, questions are one-at-a-time CRUD in v1); plagiarism/cheating detection; adaptive difficulty; student accounts or login (attempts stay token-based); PDF export of results; Razorpay recurring billing (manual UPI + admin activation, matching this challenge's day-1 approach); negative-marking beyond a flat per-question value.
